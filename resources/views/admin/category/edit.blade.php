              ...
              <form method="POST" action="{{ route('admin.category.update',$cat->id) }}" enctype="multipart/form-data">
                ...
                @method('PATCH')
                    ...
                    {{--<div class="form-group{{ $errors->has('keywords') ? ' has-error' : '' }}">
                      ...
                      <select data-placeholder="Select a keywords" class="form-control select" name="keywords[]" multiple="true">
                        @if(count($keywords)>0)
                          @foreach($keywords as $keyword)
                            <option value="{{$keyword->id}}" @if(old('keywords',$selected_ids) && in_array($keyword->id,old('keywords',$selected_ids))) selected="selected" @endif>{{$keyword->keyword}}</option>
                          @endforeach
                        @endif
                      </select>
                      ...
                      <label>{{ __('Name:')}}</label>
                      <input type="text" class="form-control" placeholder="Name" name="name" value="{{old('name',$cat->name)}}">
                      @if ($errors->has('name'))
                          <span class="help-block">
                    ...
                    <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                      ...
                      <textarea name="description" id="description" class="form-control">{{old('description',$cat->description)}}</textarea>
                    ...
                    <div class="form-group{{ $errors->has('image') ? ' has-error' : '' }}">
                      ...
                      @endif
                      @if(!empty($cat->getImgUrl()))
                        <img src="{{$cat->getImgUrl()}}" class="img-preview">
                      @endif
                    </div>
